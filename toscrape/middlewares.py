...
class DownloaderControlMiddleware:        
    def process_response(self, request, response, spider):
        ...
        # - or raise IgnoreRequest
        print('***')        
        #print (spider.settings.get('CONTROL_XPATH') )
        ...
        if bool(int(response.xpath(spider.settings.get('CONTROL_XPATH')).get())) :
            ...
            os.system("beep -f 555 -l 999") # Linux only beep sys func must be installend. Like: apt get install beep
            input("Press Enter to continue...")
            response = HtmlResponse(
                response.request.meta['driver'].current_url,
                body=response.request.meta['driver'].page_source,  # body
                encoding='utf-8',
                request=request
        )
